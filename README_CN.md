...
| `--resolution` | 选择 | *（原始）* | 输出分辨率：`2160p`、`1440p`、`1080p`、`720p`、`480p`、`360p`、`240p` |
| `--bitrate` | 字符串 | *（CRF）* | 视频码率，如 `2M`、`500k` |
| `--video-fps` | 选择 | *（原始）* | 输出帧率：`60`、`30`、`25`、`24`、`15` |
...
激进压缩视频以获得最小文件体积，同时保持可接受的画质。使用 H.264 Baseline 确保最大 Android/设备兼容性。

| 参数 | 默认值 | 说明 |
|---|---|---|
| CRF | 28 | 比 android 预设更高，压缩更强 |
| 视频编码 | H.264 (Baseline) | Level 3.1，yuv420p 像素格式 |
| 音频编码 | AAC | 单声道，64 kbps |
| 最大高度 | 720p | 源视频超过 720p 时自动缩小 |

- H.264 编码器链：h264_nvenc > h264_qsv > h264_amf > h264_vaapi > h264_videotoolbox > libx264
- 单声道低码率音频进一步缩减体积
